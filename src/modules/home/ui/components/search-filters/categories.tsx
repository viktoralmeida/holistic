...
useEffect(() => {
    ...
    calculateVisible(); // Call once on mount

    const ro1 = new ResizeObserver(calculateVisible);
    const ro2 = new ResizeObserver(calculateVisible);

    if (containerRef.current) ro1.observe(containerRef.current);
    if (measureRef.current) ro2.observe(measureRef.current);
